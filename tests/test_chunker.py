...
def test_jaccard_identical_sets():
    s = frozenset(["logD", "salacia", "batch"])
    assert _jaccard(s, s) == 1.0


def test_jaccard_disjoint_sets():
    a = frozenset(["logD", "salacia"])
    b = frozenset(["pKa", "acidic"])
    assert _jaccard(a, b) == 0.0


def test_jaccard_partial_overlap():
    a = frozenset(["logD", "salacia", "batch"])
...
def test_content_words_filters_stop_words():
    words = _content_words("this is just about logD prediction from ACAS")
...
def _logd_msg(i: int) -> str:
    """Clearly logD-domain message — no words shared with pKa domain (unique LDMSG prefix)."""
    return f"lipophilicity salacia chromatographic octanol partition coefficient outlier batch LDMSG{i:04d}"
...
def test_topic_shift_all_messages_covered():
    ...
    for i in range(len(logd_msgs)):
        assert f"LDMSG{i:04d}" in all_text
