...
def test_parse_session_indexes_assistant_text(tmp_path):
    ...
    lines = [
        json.dumps(claude_user("what compounds were suspicious in CompoundX?")),
        json.dumps({
            ...
            "message": {
                "role": "assistant",
                "content": [
                    {"type": "text", "text": "CHEMBL12345 has delta logD of +6.11, four sigma above batch mean. Almost certainly a measurement error from the ExternalCRO batch."},
                    {"type": "tool_use", "name": "Bash", "input": {"command": "echo hi"}},
                ],
            },
        }),
    ]
    path = tmp_path / "compoundx.jsonl"
    path.write_text("\n".join(lines))

    result = parse_session(path)
    assert result is not None
    assert "CHEMBL12345" in result.all_user_text
    assert "four sigma" in result.all_user_text
    assert "Bash" not in result.all_user_text  # tool_use excluded
    assert any("CHEMBL12345" in m for m in result.messages)
...
def test_extract_assistant_text_extracts_text_blocks():
    msg = {"content": [
        ...
        {"type": "text", "text": "The result confirms CHEMBL12345 is an outlier."},
    ]}
    result = _extract_assistant_text(msg)
    assert "Here is my analysis" in result
    assert "CHEMBL12345 is an outlier" in result
